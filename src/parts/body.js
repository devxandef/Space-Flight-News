...
function Body() {
  ...
  const pagination = async () => {
    setLoading(true);
    const response = await axios.get(
      `https://api.spaceflightnewsapi.net/v3/articles?_start=${allArticles.length}`
    );
    ...
    <div className="contnainer-body">
      <div className="header">
        <div className="header-controls">
          ...
          <Autocomplete
            ...
            sx={{ width: 300 }}
            renderInput={(params) => (
              <TextField {...params} label={"Filtrar"} />
            )}
          />
        </div>
      </div>
      ...
      >
        {allArticles.map((item, key) =>
          key % 2 === 0 ? cardLeft(item, key) : CardRight(item, key)
        )}
      </div>
            ...
            <div style={{ display: "flex", flexDirection: "row", padding: 30 }}>
              ...
              <div className="description">
                ...
                <div style={{ marginTop: 5 }}>
                  <Button
                    onClick={() =>
                      (window.location.href = `${itemSelected.url}`)
                    }
                    variant="contained"
                    size="small"
                    style={{ backgroundColor: "#302e53" }}
